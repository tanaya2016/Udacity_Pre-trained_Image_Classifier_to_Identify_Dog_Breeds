...
for idx in range(0, len(in_files), 1):
    ...
    if in_files[idx] not in results_dic:
         ...
         pet_label1 = ""
         pet_label2 = ""
         pet_label = in_files[idx].strip().lower().replace('_',' ')
         pet_label1 = re.sub('[0-9]','',pet_label)
         pet_label2 = re.sub(r'\.jpg$', '', pet_label1)
         pet_label2 = pet_label2.strip().split('\n')
         #print(pet_label2[0])
         #print(len(pet_label2))
         key = in_files
         value = pet_label2
         #for k, v in zip(key, value):
            #results_dic[key] = value
        
         #results_dic = {k:v for k, v in zip(key,value)}
         #print(results_dic)
         #results_dic[in_files[idx]] = [pet_label2[idx]]
